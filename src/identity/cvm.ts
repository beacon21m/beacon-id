...
function looksLikeCompressedSecpHex(s: string): boolean {
  return /^[0-9a-fA-F]{66}$/.test(s || '') && (/^02|03/i).test(s.slice(0,2));
}

// --- Configuration ---
function parseRelays(s: string | undefined | null): string[] {
  const raw = (s || '').trim();
  if (!raw) return ["wss://cvm.otherstuff.ai"];
...
export async function sendPaymentConfirmation(
  ...
  if (!isHex64(brainPubKey)) {
    console.error('[CVM Client] BRAIN_CVM_PUBLIC_KEY must be 64-char hex (Nostr pubkey). If you provided a 66-char 02/03-compressed key, convert it to a 32-byte hex pubkey.');
    return;
  }
  const signer = new PrivateKeySigner(privateKey);
  const relayPool = new ApplesauceRelayPool(parseRelays(getEnv('CVM_RELAYS', '')));
...
export async function startCvmServer() {
  ...
  const privateKey = getEnv('IDENTITY_CVM_PRIVATE_KEY', '').trim();
  if (!privateKey || privateKey.startsWith('YOUR_') || !/^[0-9a-fA-F]{64}$/.test(privateKey)) {
  ...
  const serverNpub = toNpub(serverPubkey);
  console.log(`[CVM] Identity Server Public Key (hex): ${serverPubkey}`);
  console.log(`[CVM] Identity Server npub: ${serverNpub}`);
  console.log("[CVM] Connecting to relays...", relays);
  ...
  mcpServer.registerTool(
    ...
    async (args) => {
      ...
      const webUser2 = findGatewayUserByNpubForType(args.npub, 'web');
      const targetUser = webUser2 || findGatewayUserByNpub(args.npub);
      const targetGateway: 'web' | 'whatsapp' | null = webUser2 ? 'web' : (targetUser ? 'whatsapp' : null);
  ...
  // -------------------- receiveMessage tool (moved from Brain; Identity-only install) --------------------
  const ReceiveMessageSchema = {
    refId: z.string().min(3),
    returnGatewayID: z.string().min(3),
  ...
  mcpServer.registerTool(
    'receiveMessage',
    {
      ...
      description: 'Enqueue a message into the Identity processing pipeline while preserving full routing context',
      inputSchema: ReceiveMessageSchema,
    },
    async (args) => {
      try {
        console.log('[identity-cvm] receiveMessage <-', {
        ...
        const normalizedUser = userIdRaw;
        ...
        console.log('[identity-cvm] mapping lookup', {
        ...
        // Persist inbound and remember routing
        const { recordInboundMessage } = await import('../db');
        const inboundMessageId = recordInboundMessage(beacon);
        // Enqueue for Identity processing
        enqueueIdentityBeacon(beacon);
        console.log('[identity-cvm] inbound -> enqueued identity beacon', { beaconID: beacon.beaconID });
  ...
  // -------------------- sendMessage tool (allow CVM clients to request delivery via configured gateway) --------------------
  mcpServer.registerTool(
    'sendMessage',
    {
      title: 'Send Message',
      description: 'Request Identity to send a message to a user via the configured CVM gateway dispatcher',
      inputSchema: {
        networkID: z.string().min(2),
        userId: z.string().min(1),
        message: z.string().min(1),
        returnGatewayID: z.string().optional(),
    ...
    async (args) => {
      try {
        ...
        const returnGatewayID = String(args.returnGatewayID || '').trim();
        if (!networkID || !userId || !message) {
          return { status: 'failure', description: 'error: MISSING_FIELDS' } as const;
        }
        // Enqueue to outbound; identity dispatcher will deliver via CVM
        const gatewayType = mapNetworkToGatewayType(networkID);
        if (!gatewayType) return { status: 'failure', description: 'error: UNKNOWN_NETWORK' } as const;
        enqueueIdentityOut({
          to: userId,
        ...
        console.error('[identity-cvm] sendMessage error', err);
        return { status: 'failure', description: 'error: INTERNAL' } as const;
      }
    }
  );
