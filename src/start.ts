...
function main() {
  ...
  if (!npub) {
    console.warn('[start] GATEWAY_NPUB is not set; running in CVM mode only');
  }

  // Assume CVM mode only; do not start any local adapters
  const mode = 'cvm';
  console.log('[start] CVM-only mode enabled (no local gateways)');
